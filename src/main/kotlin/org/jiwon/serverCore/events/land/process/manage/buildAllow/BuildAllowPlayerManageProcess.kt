...
class BuildAllowPlayerManageProcess(val player: Player,val landName:String):PlatformTaskProcess {
    ...
    override fun bedrockRun() {
        floodgateInstance.getPlayer(player.uniqueId).sendForm(
            LandManager.landBuildAllowPlayersSettingBedrockMenu(player,landName){
                ...
                if(buttonText.indexOf("§") == -1){
                    ...
                    // 예외처리 테스트
                    LandManager.removeBuildAllowPlayer(player,landName,selectPlayer?.uniqueId.toString())
                    player.sendMessage(mm(
                        LandTexts.REMOVED_BUILD_ALLOW_PLAYER.replace(LAND_NAME,landName).replace(
                            PLAYER_NAME, selectPlayer!!.name)))
                }else{
                    floodgateInstance.getPlayer(player.uniqueId).sendForm(
                        BedrockManager.playerSelectMenu(BUILD_ALLOW_PLAYER_MENU_TITLE_BEDROCK,player){
                                it2 ->
                            val selectPlayer = Bukkit.getOnlinePlayers().find { pl -> pl.name == it2.clickedButton().text() }!!
                            LandManager.addBuildAllowPlayer(player,landName, selectPlayer.uniqueId.toString())
                            player.sendMessage(
                                mm(
                                    APPEND_BUILD_ALLOW_PLAYER.replace(PLAYER_NAME,selectPlayer.name).replace(
                                        LAND_NAME,landName))
                            )
                        }
                    )
    ...
    override fun javaRun() {
        ...
        player.openInventory(LandManager.landBuildAllowPlayersSettingMenu(player, landName))
        registerEvent(BuildAllowPlayerClickEvent(player,landName))
    }
