...
class SetBalance(player: Player, vararg args:String):BalanceSubCommand(player, *args) {
            ...
            if(getPlayer(args[1]) != null){
                if(args.size > 2){
                    if(NumberUtils.isNumeric(args[2])){
                        ...
                        player.sendMessage(mm(SUCCESS_SET_BALANCE.replace(TARGET_PLAYER_NAME,targetPlayer.name).replace(
    ...
    override fun bedrockRun(player: FloodgatePlayer, vararg args: String) {
        javaRun(instance.server.getPlayer(player.correctUniqueId)!!, *args)
    }
}