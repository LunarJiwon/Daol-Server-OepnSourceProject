...
class RemoveBalance(player: Player, vararg args:String):BalanceSubCommand(player,*args) {
            ...
            if(player.server.getPlayer(args[1]) != null){
                if(args.size > 2){
                    if(NumberUtils.isNumeric(args[2])){
                        ...
                        BalanceManager.removePlayerBalance(targetPlayer,amount)

                        player.sendMessage(
                            mm(
                                SUCCESS_REMOVE_BALANCE.replace(TARGET_PLAYER_NAME,targetPlayer.name).replace(
